...
export default function AcceptInvite({ token }) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await fetch('/nexarank/api/v1/auth/accept-invite', {
        method: 'POST',
  ...
  return (
    <div style={s.shell}>
      <div style={s.card}>
        <div style={s.brandName}>Welcome to NexaRank</div>
        {done ? (
          <>
            <div style={s.hint}>Your account is ready.</div>
            <div style={s.backWrap}><a href="/nexarank-ui/" style={s.backLinkPrimary}>Sign in now</a></div>
          </>
        ) : !token ? (
          <div style={s.error}>Missing or invalid invite link.</div>
        ) : (
          <>
            <div style={s.hint}>Set a password to activate your account.</div>
            <div style={s.field}>
...
const s = {
  ...
  brandName:{ fontSize: 18, fontWeight: 700, color: '#1a202c', marginBottom: 12, textAlign: 'center' },
  ...
  hint:     { fontSize: 13, color: '#4a5568', textAlign: 'center', lineHeight: 1.5, marginBottom: 20 },
